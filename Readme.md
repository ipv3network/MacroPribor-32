...
* [Использование](#использование)
    ...
    * [Обновление интерфейса](#обновление-интерфейса)
...
При первом включении контроллер поднимает точку доступа с названием "MacroPribor-32" и паролем "MacroPribor-32", если в его EEPROM до этого не были сохранены данные для подключения к точке доступа (обновление прошивки без предварительной полной очистки флеша не очищает настройки Wi-Fi). Подключайтесь к точке доступа "MacroPribor-32", переходите на её Captive portal и настраивайте вашу точку доступа.
Контроллер работает только с точками доступа на 2.4 ГГц.

Если нужно принудительно сбросить настройки Wi-Fi -- нажмите кнопку "Перенастроить Wi-Fi".

## Обновление интерфейса

Если нужно изменить какие-то настройки в веб-интерфейсе, можно отредактировать файл data/index.html и залить новую версию в ФС контроллера через форму для загрузки файлов. Прошивка получилась слишком толстой, чтобы нормально обновлять её через OTA. Но большую часть вещей, которые может захотеться поменять, можно изменить в html файле.